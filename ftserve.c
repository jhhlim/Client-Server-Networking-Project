...
// socketCreation(struct addrinfo * infoaddr)
// Description: creates socket and checks to make sure theres no errors with creating socket
...
int socketCreation(struct addrinfo * infoaddr){               //puts the values into socket based on arg
	int sockfd; //test connection
...
void sendFileorList(char* command, char* ip, char * portnum, char * nameFile, char** filedir, int totalFiles){                //This is what we'll use to send the file
   ...
   {
		struct addrinfo * infoaddr = srvAddresscreate(ip, portnum);      // creates addr so we can use this later on
		int datasocket;
        int currentcondition;
        datasocket = socketCreation(infoaddr); 
        if ((currentcondition = connect(datasocket, infoaddr->ai_addr, infoaddr->ai_addrlen)) == -1)
        { //connecting socket and checks for errors with connection
            fprintf(stderr, "Error in connection.\n"); //error if the connection did not work
            exit(1);
        }
        char* completed = "complete";
        //creates socket with addrinfo then connect it to server
        ...
        //read only for file descriptor  https://linux.die.net/man/3/open
        int working = 1;
		while (working) {
            void* temp = tempbuffer; //need a temporary file to add total bytes
			int totalbytes = read(fd, tempbuffer, sizeof(tempbuffer) - 1); ;
            if (totalbytes == 0){
			...
			while (totalbytes > 0) {
                int totalbytesWritten = send(datasocket, temp, sizeof(tempbuffer),0);  //referenced lines 163-188 https://github.com/TylerC10/CS372/blob/master/Project%202/ftserver.c
		...
		send(datasocket, completed, sizeof(completed),0); //sends this complete message to datasocket
        ...
        char* completed = "complete";
        int currentcondition;
		struct addrinfo * infoaddr = srvAddresscreate(ip, portnum);      //setup server addr
		datasocket = socketCreation(infoaddr);  //create socket 
        if ((currentcondition = connect(datasocket, infoaddr->ai_addr, infoaddr->ai_addrlen)) == -1)
        { //connecting socket and checks for errors with connection
            fprintf(stderr, "Error in connection.\n"); //error if the connection did not work
            exit(1);
...
void ftpAcceptClient(int fdNEW){	            //for this, referenced 209-285 for some guidance and structure from https://github.com/TylerC10/CS372/blob/master/Project%202/ftserver.c
    ...
    if(strcmp(command,"l") == 0){                  //Directory request so get the number of files and send them
        ...
        if (validfile == 1){
            printf("File has been found in directory, sending %s to client\n", filename);
            char newfilename[100];
            char * validfile = "valid";
            send(fdNEW, validfile, strlen(validfile), 0);
            memset(newfilename, 0, sizeof(newfilename));  //referenced lines 249-253 for this part from https://github.com/TylerC10/CS372/blob/master/Project%202/ftserver.c
            strcpy(newfilename, "./");  //copy over 
            char* strfile = newfilename + strlen(newfilename);
            strfile += sprintf(strfile, "%s", filename);
            sendFileorList(command, ip, portnum, newfilename, files, filenum);
        }
        else{
            printf("Server unable to find file. Sending error message over to client.\n");
            char * invalidfile = "Please exit client to try again. Unable to find file.";
            int datasocket;
            int currentcondition;
            struct addrinfo * infoaddr = srvAddresscreate(ip, portnum);      //setup server addr
            datasocket = socketCreation(infoaddr);  //create socket 
            ...
            char* completed = "complete";
            send(datasocket, invalidfile, strlen(invalidfile), 0); //sends over msg that its been complete
         //   send(datasocket, completed, sizeof(completed),0); //sends this complete message to datasocket
            close(datasocket); //listing done so close socket
            freeaddrinfo(infoaddr);  //free the addr
        //    exit(1);
